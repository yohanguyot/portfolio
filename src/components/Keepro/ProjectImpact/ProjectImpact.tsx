...
export default function ProjectImpact({ dict }: Props) {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (featureEl) {
      const card = featureEl.firstElementChild;
      if (card) Array.from(card.children as HTMLCollectionOf<HTMLElement>).forEach(item => {
  ...
  useEffect(() => {
    ...
    cleanups.push(observe(section, 0.1, () => {
      requestAnimationFrame(() => requestAnimationFrame(() => {
        ...
        const descEl = descRef.current;
        if (descEl) {
          descEl.style.transition = `opacity ${DURATION}ms ${EASE} 200ms, transform ${DURATION}ms ${EASE} 200ms`;
          descEl.style.opacity = '1';
          descEl.style.transform = 'scale(1) translateY(0)';
          setTimeout(() => { descEl.style.transform = ''; descEl.style.transition = ''; }, DURATION + 200);
        }
